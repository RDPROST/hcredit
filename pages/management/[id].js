    ...
    useEffect(() => {
        ...
        data().then(data => {
            if (data != null) {
                ...
                setSearchActivity(data.data?.find(item => item.id === router.query?.id)?.title);
            }
        })
    }, [])

    useEffect(() => {
        setSearchActivity(dataActivities.data?.find(item => item.id === router.query?.id)?.title);
    }, [router.query.id])
                                ...
                                return (
                                    <div key={index} className="management__coach-item">
                                        {(collaborator.url.length > 0) ?
                                            <Image loader={loaderImgUrl} src={collaborator.url} alt="" className="management__coach-img" width={35} height={35}/>
