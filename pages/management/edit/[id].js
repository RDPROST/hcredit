...
export default function Edit({api}) {
    ...
    useEffect(() => {
        let event;
        const data = async () => {
            return await api("/GetInfoEvent",JSON.stringify({
                id: router.query.id
            }))
                .then(res => res.data)
        ...
        data().then(data => {
            if (data != null) {
                event = data;
                setDataEvent(data);
                setDataCoach(data.trainer);
                setDataCollaborator(data.collaborators);
            }
        }).then(() => {
            dataInfo().then(data => {
                if (data != null) {
                    setDataActivities(data);
                    setSearchActivity(data.data.find(item => item.id === event?.activity_id).title);
    ...
    const onClickAddCoach = (e) => {
        if (e.target.type === "button" || e.code === "Enter") {
            if (!dataCoach.find(item => item.id === dataSearchCoach[0].id)) {
    ...
    const onClickAddCollaborator = (e) => {
        if (e.target.type === "button" || e.code === "Enter") {
            if (!dataCollaborator.find(item => item.id === dataSearchCollaborator[0].id)) {
    ...
    const onClickPrevSlide = () => {
        ...
        if (slide === 0) {
            setSlide(dataCoach.length - 1)
        }
    }

    const onClickNextSlide = () => {
        if (slide < dataCoach.length - 1) {
            setSlide(slide + 1);
        }
        if (slide === dataCoach.length - 1) {
            setSlide(0)
